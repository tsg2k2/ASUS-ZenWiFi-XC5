...
int dhd_fkb_pool_init( void ) {
    ...
    fkbpollEntrySize = (size_t)dhd_fkbpool_entry_size();
#else
    fkbpollEntrySize = DHD_FKBPOOL_ENTRY_SIZE;
#endif

    printk("DHD_FKB_POOL size is:%u and entry size:%zu\n",g_dhd_fkbpool_size, fkbpollEntrySize);
    if(g_dhd_fkbpool_size==0) return 0;
    si_meminfo(&sys_info);
    if((g_dhd_fkbpool_size * fkbpollEntrySize) >= (sys_info.freeram * PAGE_SIZE)) {
    ...
    for (i = 0; i < g_dhd_fkbpool_size; ++i) {
        fkb_p=(FkBuff_t *)(kmalloc(fkbpollEntrySize, GFP_ATOMIC));
        if(!fkb_p) {
