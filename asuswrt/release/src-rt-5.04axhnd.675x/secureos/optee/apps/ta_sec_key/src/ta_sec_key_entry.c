                                      ...
                                      uint32_t param_types, TEE_Param params[4])
{
  ...
  case TA_SK_SET_PRIV_KEY:
    ...
    if((result = sk_get_object_header(ses_handle->obj_handle, &header)) == TEE_SUCCESS){
       uint8_t key_len = (cmd_id == TA_SK_GENERATE_KEY) ? (params[1].value.a + 7) / 8 : params[1].memref.size;
       ...
       /* Save imported key and IV */
          ...
          /* Save the key */
          result |= TEE_WriteObjectData(ses_handle->obj_handle, params[1].memref.buffer, params[1].memref.size);
          /* Save the IV */
          if (params[2].memref.buffer && params[2].memref.size)
             result |= TEE_WriteObjectData(ses_handle->obj_handle, params[2].memref.buffer, params[2].memref.size);
          else{
             uint8_t null_iv[16];
  ...
  case TA_SK_SET_PUBL_KEY:
    ...
    if((result = sk_get_object_header(ses_handle->obj_handle, &header)) == TEE_SUCCESS){
       ...
       if ((result = TEE_WriteObjectData(ses_handle->obj_handle, params[1].memref.buffer, params[1].memref.size))!= TEE_SUCCESS)
          break;
    }
    break;
  ...
  case TA_SK_GET_PUBL_KEY:
    ...
    if((result = sk_get_object_header(ses_handle->obj_handle, &header)) == TEE_SUCCESS){
       ...
       if (header.publ_len) {
          ...
          result |= TEE_ReadObjectData(ses_handle->obj_handle, params[1].memref.buffer, read_bytes, &read_bytes);
  ...
  case TA_SK_CRYPTO_INIT:
    if((result = sk_get_object_header(ses_handle->obj_handle, &header)) == TEE_SUCCESS){
       ...
       if (sk_contains_symmetric_key(&header)){
          ...
          ses_handle->session = SK_SESSION_SYMMETRIC;
          ...
          result  = TEE_AllocateOperation(&ses_handle->op_handle, map_algo[params[1].value.a], map_mode[params[1].value.b], header.priv_len * 8);
          /* Create a symmetric key for AES */
          result += TEE_AllocateTransientObject(TEE_TYPE_AES, header.priv_len * 8, &ses_handle->key_handle);
  ...
  case TA_SK_CRYPTO_UPDATE:
    if (ses_handle->key_handle && ses_handle->op_handle){
       if (ses_handle->session == SK_SESSION_SYMMETRIC)
          result = TEE_CipherUpdate(ses_handle->op_handle,
                                    params[1].memref.buffer, params[1].memref.size,
                                    params[2].memref.buffer, &params[2].memref.size);
       else if (ses_handle->session == SK_SESSION_ASYMMETRIC_ENCRYPT){
          result = TEE_AsymmetricEncrypt(ses_handle->op_handle, NULL, 0,
                                         params[1].memref.buffer, params[1].memref.size,
                                         params[2].memref.buffer, &params[2].memref.size);
       }
       else if (ses_handle->session == SK_SESSION_ASYMMETRIC_DECRYPT){
  ...
  case TA_SK_CRYPTO_FINAL:
    if (ses_handle->key_handle && ses_handle->op_handle){
       result = TEE_SUCCESS;
       if(params[1].memref.buffer && params[2].memref.buffer && ses_handle->session == SK_SESSION_SYMMETRIC)
